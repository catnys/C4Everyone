...
void test1() {
    ...
    display(s1);
    std::cout << s1.count(10) << std::endl; // returns 1
    std::cout << *s1.find(5) << std::endl; // returns the value inside in it, in this case : 4
...
void test2() {
    ...
    auto interat = stooges.find(Person{ "Larry", 1 });

    std::cout << *interat << std::endl;
...
void test3() {
    ...
    display(s);
    std::cout << "----- * ----" << std::endl;
    std::cout << std::boolalpha; // for true = 1 and false = 0 value
    std::cout << "first: " << *(result.first) << std::endl;
    std::cout << "second: " << result.second << std::endl; // true : insertion available / carried out successfully
