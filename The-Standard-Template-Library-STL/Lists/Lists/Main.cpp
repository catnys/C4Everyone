...
void myTest() {
    ...
    char fillChar = '-';
    std::cout << std::setfill(fillChar); //HOLD

    // Create a line of dashes
    int lineLength = 40;
    std::cout << std::setw(lineLength) << "" << std::endl;
    ...
    auto it = std::find(stooges.begin(), stooges.end(), Person{ "Curly", 17 });

    std::cout << "*it = " << *it << std::endl;

    stooges.emplace_back("Sarah", 15);      // Use emplace_back!!! which is such an amazing useful function
    display(stooges);

    stooges.push_back(Person{ "Mark", 22 });
    display(stooges);

    stooges.push_front(Person{ "Alexandra", 26 });
    display(stooges);

    stooges.sort();
    display(stooges);

    stooges.push_front(Person{ "Kate", 44 }); // Not sorted anymore 
    display(stooges);
