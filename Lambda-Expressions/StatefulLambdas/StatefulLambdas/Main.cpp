...
void test6()
{
	...
	auto l = [=, &y]() mutable { // capture everything by ! value except y
		x += 100;
		y += 100;
		z += 100;
		std::cout << x << std::endl;
...
void test7()
{
	...
	auto l = [&, x, z]() mutable { // capture everything by reference & , but ! NOT x and z
		x += 100;
		y += 100;
		z += 100;
		std::cout << x << std::endl;
...
public:
	...
	// this, is encouraged to use
	auto change_person1() { return [this](std::string new_name, int new_age) {name = new_name; age = new_age; }; } // this->xxx doesn't need anymorre such as this->name since this kw handled by compiler
